...
// Function to spawn SRS models in a spiral pattern
void spawnSRSModelsInSpiral(ros::ServiceClient spawnClient)
{
  int numModels = 50;        // Number of SRS models to spawn
  double radius = 0.7;      // Initial distance from the center
  double angleIncrement = 0.2;  // Angle increment for each model
  ...
  for (int i = 0; i < numModels; i++)
  {
    ...
    double z = 0.409962; // Fixed height

    // Spawn the SRS model at the calculated position
    gazebo_msgs::SpawnModel req = createSRSRequest("srs_" + std::to_string(i+1), 0, x, y, z,
                                                   1.5707, 0.0, 0.524461,  // rotation
                                                   0.2, 0.2, 0.2);
    if (spawnClient.call(req))
    {
        ROS_INFO_STREAM("SRS " << i+1 << " spawned successfully!");
    }
    else
    {
        ROS_ERROR_STREAM("Failed to spawn SRS " << i+1 << "!");
    }

    // Increase the radius for the next iteration to create the spiral
    radius += 0.2;
  }
}

// Function to spawn SRS models in a square pattern
void spawnSRSModelsInSquare(ros::ServiceClient spawnClient)
{
  int numModels = 20;        // Number of SRS models to spawn
  double startX = -0.5;     // Initial x position
  double startY = -0.5;     // Initial y position
  double increment = 0.3;   // Increment for each model

  for (int i = 0; i < numModels; i++)
  {
    // Calculate the position for the current model
    double x = startX + (i % 2) * increment;
    double y = startY + (i / 2) * increment;
    double z = 0.409962; // Fixed height
...
// multiple srs initial pose
  ...
  // spawnSRSModelsInSpiral(spawnClient);

  // spawnSRSModelsInSquare(spawnClient);

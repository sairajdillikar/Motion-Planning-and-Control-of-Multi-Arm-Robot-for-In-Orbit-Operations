...
double offset_srs_x = 0.03;
// double offset_srs_y = 0.05;
                          ...
                          const std::vector<std::string>& arm2_joint_names)
{
  ...
  while (nh.ok())
  {
    ...
    arm1_pose.position.x = transformStamped.transform.translation.x + offset_srs_x;  
    ...
    arm2_pose.position.x = arm2_pose.position.x - offset_srs_x;
    arm2_pose.position.y = arm2_pose.position.y - 0.0;
    ...
    {
        ROS_INFO("Did not find IK solution for Arm1");
    }
    else if(!arm2_found_ik)
    {
        ROS_INFO("Did not find IK solution for Arm2");
    }
                          ...
                          const std::vector<std::string>& arm2_joint_names)
{
  ...
  while (nh.ok())
  {
    ...
    arm2_pose.position.x = transformStamped.transform.translation.x + offset_srs_x;
    ...
    arm1_pose.position.x = arm1_pose.position.x - offset_srs_x;
    arm1_pose.position.y = arm1_pose.position.y - 0.0;
...
void getMidpoint(geometry_msgs::Pose& arm1_pose,
                geometry_msgs::Pose& arm2_pose,
                moveit::planning_interface::MoveGroupInterface& rng_group_interface,
                moveit::core::RobotStatePtr& kinematic_state,
                const moveit::core::JointModelGroup* arm1_joint_model_group,
                ...
                const std::vector<std::string>& arm2_joint_names)
{
  ...
  gazebo_msgs::GetLinkState linkStateRequest1;
  linkStateRequest1.request.link_name = "link_06_1";
  linkStateRequest1.request.reference_frame = "origin";
  ...
  pid_constants.kp = 0.25;
  pid_constants.ki = 0.0;
  pid_constants.kd = 0.0;

  ros::Rate rate(10.0);
  while (nh.ok())
  {

    if (client.call(linkStateRequest1))
    {
        endEffectorPose1 = linkStateRequest1.response.link_state.pose;
        // ROS_INFO("link_06_1 position with respect to origin frame: x = %f, y = %f, z = %f",
        //           endEffectorPose1.position.x, endEffectorPose1.position.y, endEffectorPose1.position.z);
    }
    else
    {
        ROS_WARN("Failed to get link_06_1 position from Gazebo");
    }

    gazebo_msgs::GetLinkState linkStateRequest2;
    linkStateRequest2.request.link_name = "link_06__2__1";
    linkStateRequest2.request.reference_frame = "origin";

    if (client.call(linkStateRequest2))
    {
        endEffectorPose2 = linkStateRequest2.response.link_state.pose;
        // ROS_INFO("link_06__2__1 position with respect to origin frame: x = %f, y = %f, z = %f",
        //           endEffectorPose2.position.x, endEffectorPose2.position.y, endEffectorPose2.position.z);
    }
    else
    {
        ROS_WARN("Failed to get link_06__2__1 position from Gazebo");
    }

    geometry_msgs::Pose midpoint;
    midpoint.position.x = (endEffectorPose1.position.x + endEffectorPose2.position.x) / 2.0;
    midpoint.position.y = (endEffectorPose1.position.y + endEffectorPose2.position.y) / 2.0;
    midpoint.position.z = (endEffectorPose1.position.z + endEffectorPose2.position.z) / 2.0;

    // ROS_INFO("midpoint is: x = %f, y = %f, z = %f", midpoint.position.x, midpoint.position.y, midpoint.position.z);

    gazebo_msgs::GetLinkState linkStateRequest0;
    linkStateRequest0.request.link_name = "dummy_link";
    linkStateRequest0.request.reference_frame = "origin";

    if (client.call(linkStateRequest0))
    {
        robot_base = linkStateRequest0.response.link_state.pose;
    }
    else
    {
        ROS_WARN("Failed to get robot_base position from Gazebo");
    }
    ...
    try
    {
      transformStamped = tfBuffer.lookupTransform("dummy_link", "srs_1", ros::Time(0));
      tf2::doTransform(midpoint, midpoint_robot_coords, transformStamped);

      ROS_INFO("Midpoint (origin frame): x = %f, y = %f, z = %f", midpoint.position.x, midpoint.position.y, midpoint.position.z);
      ROS_INFO("Midpoint (robot frame): x = %f, y = %f, z = %f", midpoint_robot_coords.position.x, midpoint_robot_coords.position.y, midpoint_robot_coords.position.z);
      // ROS_INFO("midpoint: %f, midpoint robot coord: %f", midpoint, midpoint_robot_coords);
      // break;
    }
    catch (tf2::TransformException &ex) 
    {
      ROS_WARN("Failed to get the transform from origin to robot base: %s",ex.what());
    }



    double robot_base_error_x = midpoint_robot_coords.position.x - robot_base.position.x;

    // double arm_adjustment_x = robot_base_error_x / 4;

    // arm1_pose.position.x -= arm_adjustment_x;
    // arm2_pose.position.x -= arm_adjustment_x;
    // arm1_pose.position.y -= arm_adjustment_x;
    // arm2_pose.position.y -= arm_adjustment_x;


    // arm1_pose.position.x += pid_constants.kp*robot_base_error_x +  pid_constants.kd*(robot_base_error_x-previous_error)/delta_t;
    // arm2_pose.position.x = arm1_pose.position.x;
    // //arm1_pose.position.y -= arm_adjustment_x;
    // //arm2_pose.position.y -= arm_adjustment_x;
    // previous_error = robot_base_error_x;
    // ROS_INFO("base error: %f", robot_base_error_x);
    ...
    if (arm1_found_ik && arm2_found_ik)
    {
        ...
        rng_group_interface.execute(my_plan);
        ROS_INFO("found IK solution for Arm1 & Arm2 while robot base movement");
        
        break;
    }
    else if(!arm1_found_ik)
    {
        ROS_INFO("Did not find IK solution for Arm1 while robot base movement");
    }
    else if(!arm2_found_ik)
    {
        ROS_INFO("Did not find IK solution for Arm2 while robot base movement");
    }
...
int main(int argc, char** argv)
{
  ...
  // getMidpoint(arm1_pose, arm2_pose, rng_group_interface, kinematic_state,
  //           arm1_joint_model_group, arm2_joint_model_group,
  //           arm1_joint_names, arm2_joint_names);
